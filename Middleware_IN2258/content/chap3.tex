...
No ID of duplicates}}, 
\hlgray{Acceptable for} \redtext{\hlgray{idempotent(always same result) operations}}
\bluetext{\hlgray{3.AtMostOnce}}
\orangetext{
\begin{CJK*}{UTF8}{gbsn}Duplicate不重复处理， 不重复回复\end{CJK*}
\hlgray{
ID of duplicates: sequenceNum,} 
\redtext{\hlgray{Repeat resend, but No Repeat Execution}}, 
\hlgray{Acceptable for non-idempotent operations}, 
\redtext{\hlgray{No (or unknown result) when S crash}}
...
$\rightarrow$
Atomic execution, 
After S Crash and C resend, the crashed operation recovered and executed exactly once}}
\\
